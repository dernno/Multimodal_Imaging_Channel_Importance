...
def train(model, optimizer, scheduler, train_dataloader, val_dataloader, test_dataloader=None,
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_dataloader):
            ...
            slide = data['slide']
            batch_size = label.size(0)  # oder len(label)

            if use_mixup:
                if use_weighted_patients:
                    if i == 0:
                        print("#####Weighted-Patients + MIX UP:", train_patient_weights)
                    # Step 1: Prepare slides
                    slide_ids = [f"{int(s):02d}" for s in slide]  # original Reihenfolge
                    index = torch.randperm(batch_size).to(device)

                    image, label_a, label_b, lam = mixup_data(image, label, alpha=0.8, device=device)

                    slide_a = slide_ids
                    slide_b = [slide_ids[i.item()] for i in index]  # neue Reihenfolge entsprechend Mixu
...
#            score = torch.sigmoid(pred)
            if use_mixup:
                ...
                if use_weighted_patients:
                    if i == 0:
                        print("#####Weighted-Patients + MIX UP:",train_patient_weights)
                    # Schritt 1: Klassengewichtung
                    class_weights = torch.where(label == 1, pos_weight, neg_weight)

                    # Schritt 2: slide (numerisch) → zweistellige Strings wie '05'
                    slide_ids = [f"{int(s):02d}" for s in slide]  # s ist z. B. 5 → '05'

                    # Schritt 3: Hole Patienten-Gewicht für jeden Slide
                    ...
                    # Schritt 4: Kombiniertes Gewicht
        ...
        if use_retrain==False:
            print("training set......") 
            results2 = val(model, train_dataloader, mode, device, pos_weight, neg_weight, use_weighted_patients=use_weighted_patients)   #### validieren on train for hyperparameter tuning
            his2.append(results2)  
            log_data = {"Train/Loss2": results2['loss'],
        ...
        results3 = test(model, test_dataloader, device, fold_num=fold_num, mode=mode, model_arch_name=model_arch_name, mode_name = mode_name, 
                        log_dir = log_dir, test_patient_weights=test_patient_weights, use_weighted_patients=use_weighted_patients)#, run_name=run_name)
...
def val(model, dataloader, mode='BF', device='cuda', pos_weight=0.78, neg_weight=0.22, val_patient_weights=None, use_weighted_patients=False):
    ...
    #from here differences to test function:
    if use_weighted_patients:
        # Schritt 1: Klassengewichtung
        class_weights = torch.where(y == 1, pos_weight, neg_weight)

        # Schritt 2: slide ids holen
        slide = data['slide']  
        slide_ids = [f"{int(s):02d}" for s in slide]

        # Schritt 3: Patienten-Gewichte holen
        if val_patient_weights is not None:
            ...
            # fallback auf normale class weights
