...
parser.add_argument('--server', type=str, default='x')
...
def compute_integrated_gradients(model, test_dataloader, device, mode='MM'):
    ...
    if mode == 'MM':
        # Wrapper for multimodal model (7 channels → split into BF + FL)
    ...
    for batch_idx, batch in enumerate(tqdm(test_dataloader)):
        ...
        for i in range(images.size(0)):
            try:
                if mode == 'MM':
                    input_tensor = images[i:i+1]  # shape: [1, 7, H, W]
                    ...
                    attr_bf = attr[0].abs().mean(dim=(0, 2, 3))  # → shape: [3]
                    attr_fl = attr[1].abs().mean(dim=(0, 2, 3))  # → shape: [4]
                    ...
                    input_tensor = images[i:i+1]  # shape: [1, C, H, W]
                    baseline_tensor = torch.zeros_like(input_tensor)
...
def run_cv_integrated_gradients(args):
    ...
    for i in folds_to_run:
        ...
        log_path = f'{log_dir}/integrated_gradients_logs/{last_two}/{args.note}'
