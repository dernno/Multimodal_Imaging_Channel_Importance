...
def main(args):
    ...
    # Create DataLoader
    dataloader = DataLoader(filtered_dataset, batch_size=64, shuffle=False, num_workers=16)

    # Compute mean and std
    mean = torch.zeros(4).to(device)
    std = torch.zeros(4).to(device)
    count = 0
    ...
    for images, _ in tqdm(dataloader):
        ...
        images = images.view(batch_samples, 4, -1)
